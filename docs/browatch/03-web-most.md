...
> Статус: v0.2.2 — полный паритет с прошивкой + Логово (часы DESK), проверено на железе 2026-09-22.
...
- Сама плата (`USB-XXXX`, env/версия/язык/ник/скин/настройки DESK) + сквад
  рядом (снапшот каждые 5 с, у каждого `outfit`/`shade` из последней рекламы).
...
- 🏠 Логово — часы DESK с платы (таб; на десктопе — во всю ширину под колонками):
  семисегментные цифры 1:1 (SEG из `ui_desk.cpp`), персонаж платы в её скине
  по центру + сквад в своих скинах (сколько — `desk.crowd` с платы, сортировка
  по MAC как на плате), всплывающее ПИСЬМО как XP-коробка платы (тап гасит),
  облачка последних реплик, карточка свежей детекции, местный таймер
  ФОКУС 25/ПЕРЕРЫВ 5 (плату не трогает). Полоска настроек зеркалит DESK MODE
  платы (сквад/сколько/визит/часы/фон) — меняются на плате, здесь чтение.
  Без платы — часы идут, сквада нет. Проверено headless-скриншотами:
  часы + сквад + письмо + детекция на живой связке USB-1388/SHADOW.
- Не зеркалится (локально на плате): бинго-карта, дневник, настройки,
  ростер/счётчики встреч.
