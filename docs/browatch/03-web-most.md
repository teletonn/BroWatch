...
## Персона: только персона платы (v0.2.1)

У веба своей персоны нет и быть не может: имя, «Я тут», heartbeat и
веб-локальные отправки убраны. Всё сказанное в вебе уходит в эфир через
плату от её персонажа — `nick` из announce (`payphone NAME`, иначе индексный
ник; плата 2 сейчас — `SQUACHY`). Сервер игнорирует `from` от клиента,
без платы в эфире отвечает `503 board offline`, `POST /api/peers` закрыт
(410). Члены сквада приходят только по эфиру — добавить вручную нельзя.
