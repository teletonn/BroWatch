...
2. **Коммуникатор (SquachMesh).** Платы в радиусе BLE-видимости образуют «сквад»:
...
- Сообщения: `Settings → SQUACHMESH → MESSAGES → PHRASE` (5 слов; ROLL/ENTER; ~3 с PBKDF2). 24 готовые строки или до 48 символов с клавиатуры. Красный пузырь, подтверждение перед отправкой, read receipts.
...
- `browatch-app`: нативный Kotlin + BLE-сканер SquachMesh.
